...
func (p *CurrentWeatherData) GetByCityAndCountryCode(city, countryCode string) (weather *Weather, err error) {
	return p.doRequest(fmt.Sprintf("http://api.openweathermap.org/data/2.5/weather?lat=%s&lon=%s&APPID=%s", city, countryCode, p.APIkey))
}

func (p *CurrentWeatherData) GetByGeoCoordinates(lat, lon float32) (weather *Weather, err error) {
	return p.doRequest(fmt.Sprintf("http://api.openweathermap.org/data/2.5/weather q=%f,%f&APPID=%s", lat, lon, p.APIkey))
}
